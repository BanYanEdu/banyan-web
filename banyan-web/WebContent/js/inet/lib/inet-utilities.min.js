...
iNet.ui.dialog.ModalDialog=function(a){a=a||{};this.prefix=a.prefix?a.prefix:a.id;this.id="modal-dialog";iNet.apply(this,a);this.prefix=this.prefix?this.prefix+"-":"";this.dialogId=String.format("#{0}",this.id);this.buttons=this.buttons?this.buttons:[];this.title=this.title||"";this.content=this.content||"";var d=this;if($(this.dialogId).length==0)$("body").append(String.format('<div id="{0}" class="modal fade in" style="display: none"></div>',d.id));else this.content=$(this.dialogId).html();(function(){var f=
'<div class="modal-header"><a class="close" data-dismiss="modal">&times;</a>'+String.format("<h4>{0}</h4>",d.title)+"</div>"+String.format('<div class="modal-body">{0}</div>',d.content)+'<div class="modal-footer">',j="";if(iNet.isArray(d.buttons))for(var k=0;k<d.buttons.length;k++){var p=d.buttons[k];p.id=p.id?p.id:d.prefix+iNet.generateId();j+=String.format('<button id="{0}" class="btn btn-small {1}"><span class="{2}"></span> {3}</button>',p.id,p.cls||"",p.icon,p.text);$(d.dialogId).off("click",
"#"+p.id).on("click","#"+p.id,p.fn.createDelegate(d))}var u=f+j+"</div>";$(d.dialogId).html(u)})();this.show=function(){if(!($(this.dialogId).length<1)){$(this.dialogId).modal("show");var f=this.getButtons();f.length>0&&$(f[0]).focus();return this}};this.hide=function(){$(this.dialogId).modal("hide");return this}};
...
$(function(){FormUtils={showButton:function(a,d){var f=$(a),j="";if(d){j=f.attr("style");iNet.isEmpty(j)||(j=j.replace(/display: none !important;/g,""));f.removeAttr("disabled").removeClass("hide").show()}else{f.attr("disabled","disabled").hide();j=f.attr("style");iNet.isEmpty(j)||(j=j.replace(/display: none;/g,"display: none !important;"))}f.attr("style",j)},disableButton:function(a,d){d?$(a).attr("disabled","disabled"):$(a).removeAttr("disabled")}}});
...
hi_btnhov:"ui-state-hover",hi_hnd:"ui-icon ui-icon-grip-dotted-horizontal"},bootstrap:{container:"alert",notice:"",notice_icon:"icon-exclamation-sign",info:"alert-info",info_icon:"icon-info-sign",success:"alert-success",success_icon:"icon-ok-sign",error:"alert-error alert-danger",error_icon:"icon-warning-sign",closer:"icon-remove",pin_up:"icon-pause",pin_down:"icon-play",hi_menu:"well",hi_btn:"btn",hi_btnhov:"",hi_hnd:"icon-chevron-down"}},u=function(){j=a("body");k=a(window);k.bind("resize",function(){f&&
clearTimeout(f);f=setTimeout(a.pnotify_position_all,10)})};document.body?u():a(u);a.extend({pnotify_remove_all:function(){var h=k.data("pnotify");h&&h.length&&a.each(h,function(){this.pnotify_remove&&this.pnotify_remove()})},pnotify_position_all:function(){f&&clearTimeout(f);f=null;var h=k.data("pnotify");if(h&&h.length){a.each(h,function(){var i=this.opts.stack;if(i){i.nextpos1=i.firstpos1;i.nextpos2=i.firstpos2;i.addpos2=0;i.animation=true}});a.each(h,function(){this.pnotify_position()})}},pnotify:function(h){var i,
...
iNet.ui.form.Uploader=function(a){var d=a||{};iNet.apply(this,d);this.id=this.id||"iuploader-form";this.name=this.name||"FileUpload";this.element=$("#"+this.id)||null;this.action=this.action||"";this.params=this.params||{};this.allowedExt=this.allowedExt||[];this.sizeLimit=this.sizeLimit||0;this.multiple=this.multiple||true;this.messages=this.messages||{typeError:"{file} has invalid extension. Only {extensions} are allowed.",sizeError:"{file} is too large, maximum file size is {sizeLimit}.",minSizeError:"{file} is too small, minimum file size is {minSizeLimit}.",
emptyError:"{file} is empty, please select files again without it.",onLeave:"The files are being uploaded, if you leave now the upload will be cancelled."};this.debug=this.debug||false;this.onSubmit=this.onSubmit||iNet.emptyFn;this.onProgress=this.onProgress||iNet.emptyFn;this.onComplete=this.onComplete||iNet.emptyFn;this.onCancel=this.onCancel||iNet.emptyFn;this.showMessage=function(j){alert(j)};this.input=this._createInput();var f=this;this.input.change(function(){f._validateFile(this)&&f._upload()})};
