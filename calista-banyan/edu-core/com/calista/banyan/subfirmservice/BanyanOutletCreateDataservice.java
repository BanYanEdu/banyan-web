...
import com.inet.xportal.nosql.web.data.AddressDTO;
...
/**
 * 
 * BanyanOutletCreateDataservice.
 *
 * @author Hien Nguyen
 * @version $Id: BanyanOutletCreateDataservice.java Oct 13, 2020 3:19:56 PM nguyen_dv $
 *
 * @since 1.0
 */
@Named("banyanoutletcreate")
@XPortalDataService(roles={BanyanConstant.ROLE_ADMIN}, description="Banyan Outlet service")
@XPortalPageRequest(uri="banyan/outlet/create", 
	inherit = true,
	transaction = true,
	result = WebConstant.ACTION_XSTREAM_JSON_RESULT)
public class BanyanOutletCreateDataservice extends PluginSubfirmAbstraction {
	@Inject
	...
	{
		FirmInformation profile = loadOrganization(site, firm, action);
		
		String industry = XParamUtils.getString("industry", params, profile.getIndustry());
		
		// service register
		FirmOutlet outlet = outletBO.outletRegister(profile,
				getGeoAddress(params),
				StringUtils.hasLength(industry) ? CollectionUtils.asList(industry.split(",")) : null, 
				action.getModel(AddressDTO.class), 
				params);
